...
public class FriendsController {
    ...
    @GetMapping
    @Operation(summary = "Listar amigos paginados", description = "Obtiene la lista de amigos del usuario de forma paginada con información del usuario")
    public PagedResponseDto<FriendResponseDto> getFriendsPaginated(
            @AuthenticationPrincipal String userId,
            @ParameterObject FindFriendDto findFriendDto) {

        Page<FriendResponseDto> page = crudFriendsService.getFriendsPaginated(userId, findFriendDto.getName(),
                findFriendDto.toPageable());
        return new PagedResponseDto<>(page);
    }

    @DeleteMapping("/{friendId}")
    @Operation(summary = "Eliminar amigo", description = "Elimina la relación de amistad con un usuario")
