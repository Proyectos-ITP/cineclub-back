...
public class FriendRequestsController {
    ...
    public ResponseEntity<ApiResponse<FriendRequest>> sendFriendRequest(
        ...
        FriendRequest friendRequest = crudFriendsRequests.sendFriendRequest(userId, createFriendDto.getFriendId());
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.created("Solicitud de amistad enviada exitosamente", friendRequest));
    }

    @PutMapping("/accept/{senderId}")
    @Operation(summary = "Aceptar solicitud de amistad", description = "Acepta una solicitud de amistad recibida y crea la relación de amistad")
    public ResponseEntity<ApiResponse<String>> acceptFriendRequest(
            @PathVariable String senderId,
            @AuthenticationPrincipal String userId) {

        crudFriendsRequests.acceptFriendRequest(userId, senderId);
        return ResponseEntity.ok(ApiResponse.success("Solicitud de amistad aceptada exitosamente. Ahora son amigos.", null));
    }

    @PutMapping("/reject/{senderId}")
    @Operation(summary = "Rechazar solicitud de amistad", description = "Rechaza una solicitud de amistad recibida")
    public ResponseEntity<ApiResponse<String>> rejectFriendRequest(
            @PathVariable String senderId,
            @AuthenticationPrincipal String userId) {

        crudFriendsRequests.rejectFriendRequest(userId, senderId);
        return ResponseEntity.ok(ApiResponse.success("Solicitud de amistad rechazada exitosamente", null));
    }

    @GetMapping("/received")
    @Operation(summary = "Ver solicitudes recibidas", description = "Obtiene todas las solicitudes de amistad recibidas pendientes")
    ...
    @GetMapping("/sent")
    @Operation(summary = "Ver solicitudes enviadas", description = "Obtiene todas las solicitudes de amistad enviadas por el usuario")
    ...
    @Operation(summary = "Cancelar solicitud de amistad", description = "Cancela una solicitud de amistad enviada")
    public ResponseEntity<ApiResponse<String>> cancelFriendRequest(
        ...
        return ResponseEntity.ok(ApiResponse.success("Solicitud de amistad cancelada exitosamente", null));
    }
}
