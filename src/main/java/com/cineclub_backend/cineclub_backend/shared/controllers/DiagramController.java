...
public class DiagramController {
    ...
    public ResponseEntity<String> getDiagram() {
        ...
        MutableGraph graph = mutGraph("API")
                ...
                .graphAttrs().add("layout", "neato")  // neato distribuye en círculo/radial
                .graphAttrs().add("overlap", "false")
                .graphAttrs().add("splines", "true")
                .graphAttrs().add("bgcolor", "transparent")
                .graphAttrs().add("pad", "1.0")
                .graphAttrs().add("sep", "+25")  // Separación entre nodos
                .nodeAttrs().add("fontsize", "12")
                .nodeAttrs().add("fontname", "Arial")
                .nodeAttrs().add("height", "0.7")
                .nodeAttrs().add("width", "2.5")
                .linkAttrs().add("color", "999999")
                .linkAttrs().add("penwidth", "1.5");
        ...
        controllerNodes.values().forEach(c -> {
            graph.add(c);
        ...
        for (String secName : securitySchemes) {
            ...
            graph.add(secNode.addLink(to(apiGateway).with(Color.rgb("#E74C3C"), Style.DASHED).with("penwidth", "2.0")));
        }
        ...
        controllerNodes.values().forEach(c -> {
            graph.add(c.addLink(to(mongo).with(Color.rgb("#4DB33D"), Style.DASHED).with("penwidth", "1.5")));
            graph.add(c.addLink(to(redis).with(Color.rgb("#DC382D"), Style.DASHED).with("penwidth", "1.5")));
        });

        // Renderizar con proporción más 
        String svg = Graphviz.fromGraph(graph)
                .width(1200)
                .render(Format.SVG)
                .toString();
